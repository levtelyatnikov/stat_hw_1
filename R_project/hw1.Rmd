...
for (j in 1:N){
  ...
  Player_1 = "RRR"
  Player_2 = "BRR"
  deck <- sample(c(rep("R", 26) , rep("B", 26))) # permutation the deck 
  i =1
  while (i<=(n_cards-2)) {
    c = paste(deck[i:(i+2)],collapse='')
...
### 1 
Let's simulate the algorithm 1000 times and emphasize some interesting aspects
```{r echo=FALSE}
set.seed(23123)
N = 20
n = 1000 # time steps
...
We can see that if we are creating the matrix L(p x d) dimensional every simulation we can achieve the JL lemma results.
As we can see in the algorithm some parameters which we can tune:
...
As we can impact on the JL probability with help of n and epsilon where and impact on the formula for p
